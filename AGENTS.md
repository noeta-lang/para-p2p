...
- `cargo test` inside each `crates/*` member works standalone. The two CONTRACT crates (`noeta-ext-abi`, `noeta-reactive-abi`) come from **crates.io** on a `"0.5"` range — they are published and semver-stable, so a patch toolchain release must not cost this repo a manifest edit. Every other toolchain crate is internal and unpublished, so it stays a git dependency on `https://github.com/noeta-lang/noeta` pinned to the exact release tag; CI asserts that tag equals `NOETA_VERSION`. `crates/noeta-para-p2p` also carries a `[patch.crates-io]` redirecting the two contract crates onto that same git checkout: its git-pinned dev-dependencies resolve the contract through the toolchain repo's own path copy, and without the patch a test build links two copies of the ABI and every `impl Extension` fails E0277. Cargo honors `[patch]` only in the workspace root it builds, so it governs this repo's tests and is ignored by consumers. Plain builds/tests link **no** p2panda — the extension uses its dep-free loopback broker; `cargo test --features ring-p2p` in `crates/noeta-para-p2p` builds **and runs** the real-transport ring. CI gates that, not merely `cargo check` — a `check` never builds a test target, so ring-only behavior would otherwise have zero automated coverage. The `.noe` conformance fixtures stay on the loopback broker even under that feature: they run on `SandboxHost`, whose `real_p2p()` is `None`, so no p2panda node ever starts in a test.
...
`ci.yml` gates the Rust crates (fmt/clippy/test + the `ring-p2p` check, mirroring the monorepo's old gate) and the example (pinned released `noeta`); `release.yml` re-runs the crate gate then publishes the tag to the hosted registry (`noeta publish`, keyless Sigstore provenance via GitHub OIDC).
